...
export const BaseNode = forwardRef<HTMLDivElement, BaseNodeProps>(
  ({ className, status, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "relative rounded-sm border bg-card text-card-foreground border-muted-foreground hover:bg-accent",
        className
      )}
      tabIndex={0}
      {...props}
    >
      ...
      {status === "loading" && (
        <Loader2Icon
          className="size-2 absolute -right-0.5 -bottom-0.5 text-blue
        -700 stroke-3 animate-spin"
        />
      )}
    </div>
  )
